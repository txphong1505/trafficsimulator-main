...
public class DiagnosticRunner {
    ...
    //   Bottom crosswalk: fillRect(inter.x + off, roadStartY + roadWidth + 10, 14, 30)
    ...
    // Right crosswalk far edge (vehicle coming WEST must stop after inter.x + roadWidth + 40)
    ...
    private static void probeStopLine(int tick, Vehicle v, List<Intersection> intersections) {
        if (v.getSpeed() != 0 || stopProbeCount >= 20) return;
            ...
            if (!inter.light.canGo(d)) {
                ...
                if (relevant && !Double.isNaN(mathStopLine)) {
                    double mathErr   = frontX != Double.NaN && d == Direction.EAST || d == Direction.WEST
                                       ? frontX - mathStopLine : frontY - mathStopLine;
                    double visualErr = d == Direction.EAST || d == Direction.WEST
                                       ? frontX - visualStopLine : frontY - visualStopLine;
    ...
    // Logs the pixel gap between the nearest diagonal pair and the effective predRect
    ...
    private static void probeDiagonalGap(int tick, Vehicle currentV, List<Vehicle> vehicles) {
        ...
        for (Vehicle other : vehicles) {
            if (other == currentV || other.getDirection() != d) continue;
            ...
            if (diagLogged.contains(key)) continue;
    ...
    private static void probeRenderDimensions(int tick, Vehicle v, List<Intersection> intersections) {
        if (renderProbeCount >= 20) return;
        int id = System.identityHashCode(v);
        if (renderLogged.contains(id)) return;
        ...
        for (Intersection inter : intersections) {
            ...
            if (interBox.intersects(v.getHitbox())) { inIntersection = true; break; }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  Bottom crosswalk Y: [" + CROSSWALK_BOTTOM_START_Y + ", " + (CROSSWALK_BOTTOM_START_Y + 30) + "]");
        ...
        System.out.println("  Right  crosswalk X: [inter.x+" + (ROAD_WIDTH + 10) + ", inter.x+" + (ROAD_WIDTH + 40) + "]");
        System.out.println();
    ...
    private static void runProbeRun(int totalTicks, double spawnRate) {
        stopLogged.clear(); stopProbeCount = 0;
        diagLogged.clear(); diagProbeCount = 0;
        renderLogged.clear(); renderProbeCount = 0;
    ...
    private static void runTest(int totalTicks, double spawnRate, double emgBoost, int testNum) {
        ...
        for (int tick = 1; tick <= totalTicks; tick++) {
            ...
            if (Math.random() < eff) controller.spawnVehicle();
            controller.updateAll();
            if (vehicles.size() > localMax) localMax = vehicles.size();

            List<Vehicle> vList = new ArrayList<>(vehicles);
            for (int i = 0; i < vList.size(); i++) for (int j = i+1; j < vList.size(); j++) {
                ...
                if (a.getShrinkHitbox(3).intersects(b.getShrinkHitbox(3)) && a.getDirection() != b.getDirection()) {
                    ...
                    if (loggedCollisions.add(k)) {
                        localCol++;
                        if (localCol <= 5) System.out.printf("  [Tick %d] COLLISION: %s(%s) <-> %s(%s)%n",
                ...
                if (v.getSpeed() == 0 && !controller.isWaitingForLight(v)) {
                    ...
                    if (localStopped.getOrDefault(id,0) == 101) {
                        if (loggedDeadlocks.add(String.valueOf(id))) {
                            localDead++;
                            if (localDead <= 5) System.out.printf("  [Tick %d] DEADLOCK: %s at (%.0f,%.0f) dir=%s%n",
                                tick, v.getName(), v.getX(), v.getY(), v.getDirection());
                        }
                    }
                } else { localStopped.put(id, 0); }
            }
            if (tick % 1000 == 0) System.out.printf("  [Tick %d] v=%d col=%d dead=%d%n",
        ...
        collisionCount += localCol; deadlockCount += localDead;
        loggedCollisions.clear(); loggedDeadlocks.clear();
    }
}
